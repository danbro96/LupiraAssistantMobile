import { deviceKeyPort, type ApiBase } from './auth-ports';
...
async function authedFetch(path: string, init: RequestInit, base: ApiBase): Promise<Response> {
  ...
  const url = joinUrl(port.getApiUrl(base), path);
...
// Phase 2 (smart ring) — stays on HealthApi; needs its own HealthApi device key once wired.
...
/** Highest location seq the server has accepted (resume cursor). Device is derived server-side from the key. */
